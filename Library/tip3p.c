...
new_tip3p (uint n) {
  ...
  pmolecule *mol;
  pfield q;
  ptip3p t = (ptip3p) allocmem (sizeof (tip3p));
...
inputfile_tip3p (const char *file) {
  ...
  for (i=0; i<n; i++) {
    ...
    xc[j][0] = xn[i][0] = 1e-10 * xo;
    xc[j][1] = xn[i][1] = 1e-10 * yo;
    xc[j][2] = xn[i][2] = 1e-10 * zo;
    xcmol[j] = xnmol[i] = i+1;
...
update_tip3p (pspatialgeometry sg, pkernelmatrix kc, pkernelmatrix klj, 
  ...
  // Recalculate positions of centers of mass and atoms, respectively
  for (i=0; i<n; i++) {
    k = 3 * i;
    com = mol[i]->com;
    r = mol[i]->r[0];
    for (j=0; j<3; j++) {
      xc[k][j] = xn[i][j] = com[j] + r[j];
    }
  ...
  calcForce_tip3p (fc, flj, t);
  calcTorque_tip3p (fc, flj, t);
}
...
velocityVerlet_tip3p (pspatialgeometry sg, pkernelmatrix kc, pkernelmatrix klj, 
  ...
  update_tip3p (sg, kc, klj, Fc, Flj, fc, flj, t);
  E = 0.0;
  for (i=0; i<n; i++) {
